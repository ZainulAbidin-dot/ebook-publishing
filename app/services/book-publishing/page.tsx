...
export default function BookPublishingPage() {
    const serviceTitle = "Book Publishing";
    const serviceDescription = "A seamless journey from manuscript to global marketplaces with our end-to-end publishing solutions.";
                ...
                <div className="container">
                    <div className="grid grid-2" style={{ alignItems: 'center', gap: '4rem' }}>
                        <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
                            <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: '2.5rem', color: 'var(--primary)', marginBottom: '1.5rem' }}>Global Reach, Local Impact</h2>
                            <p style={{ fontSize: '1.1rem', lineHeight: 1.8, color: 'var(--text-muted)', marginBottom: '2rem' }}>We take the complexity out of publishing. Our distribution network puts your book in the hands of readers worldwide, handling everything from ISBN registration to retail setup.</p>
                            <ul style={{ display: 'flex', flexDirection: 'column', gap: '1rem', listStyle: 'none', padding: 0 }}>
                                {["Amazon and IngramSpark setup", "ISBN and copyright registration", "Global distribution to 40,000+ retailers", "Royalties and stock management"].map((point, idx) => (
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} style={{ position: 'relative', height: 'clamp(300px, 50vw, 500px)', borderRadius: '2rem', overflow: 'hidden', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}>
                            <Image src={BgImage} alt="Publishing Detail" fill style={{ objectFit: 'cover' }} />
                        </motion.div>
                    </div>
                </div>
            </section>
