...
export default function ProofreadingPage() {
    const serviceTitle = "Proofreading";
    const serviceDescription = "Meticulous proofreading to catch every error and ensure your book is polished to perfection.";
                ...
                <div className="container">
                    <div className="grid grid-2" style={{ alignItems: 'center', gap: '4rem' }}>
                        <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
                            <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: '2.5rem', color: 'var(--primary)', marginBottom: '1.5rem' }}>The Final Polish</h2>
                            <p style={{ fontSize: '1.1rem', lineHeight: 1.8, color: 'var(--text-muted)', marginBottom: '2rem' }}>Errors are the enemy of authority. Our proofreaders are the last line of defense, ensuring your final manuscript is free from typos, punctuation errors, and formatting glitches.</p>
                            <ul style={{ display: 'flex', flexDirection: 'column', gap: '1rem', listStyle: 'none', padding: 0 }}>
                                {["Spelling and punctuation checks", "Grammar and syntax finalization", "Formatting consistency check", "Clarity and flow optimization"].map((point, idx) => (
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} style={{ position: 'relative', height: '500px', borderRadius: '2rem', overflow: 'hidden', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}>
                            <Image src={BgImage} alt="Proofreading Detail" fill style={{ objectFit: 'cover' }} />
                        </motion.div>
                    </div>
                </div>
            </section>
