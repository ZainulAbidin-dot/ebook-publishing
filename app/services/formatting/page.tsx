...
export default function FormattingPage() {
    const serviceTitle = "Formatting";
    const serviceDescription = "Expert book formatting for both print and digital platforms, ensuring a professional reading experience.";
                ...
                <div className="container">
                    <div className="grid grid-2" style={{ alignItems: 'center', gap: '4rem' }}>
                        <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
                            <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: '2.5rem', color: 'var(--primary)', marginBottom: '1.5rem' }}>Precision in Every Page</h2>
                            <p style={{ fontSize: '1.1rem', lineHeight: 1.8, color: 'var(--text-muted)', marginBottom: '2rem' }}>We transform your manuscript into a beautifully formatted book ready for Kindle, Apple Books, and physical printing. Our attention to detail ensures your interior layout is as professional as your cover.</p>
                            <ul style={{ display: 'flex', flexDirection: 'column', gap: '1rem', listStyle: 'none', padding: 0 }}>
                                {["Custom interior layout design", "ePub and PDF conversion", "Optimized typography for readability", "Table of contents and front matter"].map((point, idx) => (
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} style={{ position: 'relative', height: '500px', borderRadius: '2rem', overflow: 'hidden', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}>
                            <Image src={BgImage} alt="Formatting Detail" fill style={{ objectFit: 'cover' }} />
                        </motion.div>
                    </div>
                </div>
            </section>
