...
export default function BookMarketingPage() {
    const serviceTitle = "Book Marketing";
    const serviceDescription = "Strategic marketing plans designed to build buzz and drive sales through multi-channel promotion.";
                ...
                <div className="container">
                    <div className="grid grid-2" style={{ alignItems: 'center', gap: '4rem' }}>
                        <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
                            <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: '2.5rem', color: 'var(--primary)', marginBottom: '1.5rem' }}>Amplifying Your Story</h2>
                            <p style={{ fontSize: '1.1rem', lineHeight: 1.8, color: 'var(--text-muted)', marginBottom: '2rem' }}>We don't just publish books; we build authors. Our marketing team leverages social media, PR, and digital advertising to ensure your book gets the attention it deserves.</p>
                            <ul style={{ display: 'flex', flexDirection: 'column', gap: '1rem', listStyle: 'none', padding: 0 }}>
                                {["Social media campaign management", "Targeted Amazon and Facebook ads", "Author branding and website design", "PR and media outreach sequences"].map((point, idx) => (
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} style={{ position: 'relative', height: '500px', borderRadius: '2rem', overflow: 'hidden', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}>
                            <Image src={BgImage} alt="Marketing Detail" fill style={{ objectFit: 'cover' }} />
                        </motion.div>
                    </div>
                </div>
            </section>
